...
def find_path(obs_map, init_pos, goals, model_config=None, 
              recursive=True, inflation=1.0,
    ...
    nodes_popped = 0
    corrected_graph_size = 0
    collisions = []
    max_collision_set_size = 0 
    total_collisions = 0

    import resource
    resource.setrlimit(resource.RLIMIT_AS, (2**36, 2**36))
    ...
    try:
      ...
      global_move_list = []
      o = Od_Mstar(obs_map, goals, recursive=recursive, model_config=model_config, 
                   inflation=inflation,
    ...
    corrected_graph_size = o.get_graph_size(False)
    collisions = o.collision_logger
    max_collision_set_size = o.max_collision_set_size
    total_collisions = o.total_collisions
    
    output = {}
    output['path'] = path
    output['num_nodes'] = num_nodes
    output['corrected_mem'] = corrected_mem
    output['time_taken'] = time_taken
    output['nodes_popped'] = nodes_popped
    output['corrected_graph_size'] = corrected_graph_size
    output['collisions'] = list(collisions)
    output['max_collision_set_size'] = max_collision_set_size
    output['total_collisions'] = total_collisions
...
class Od_Mstar(object):
    ...
    """
    def __init__(self, obs_map, goals, recursive, model_config,
                 sub_search=None,
        ...
        self.predictor = None
        self.nodes_popped = 0
        self.collision_logger = set()
        self.max_collision_set_size = 0
        self.total_collisions = 0
        self.odrmstar_already_predicted_next_step = set()
        
        if model_config is not None:
          print('Initialising predictor.')
          self.predictor = learnt_predictor.Predictor(model_config)
    ...
    def heuristic(self, coord, standard_node):
        ...
        """
        #print('Standard node:', standard_node)
        #print('Coord:', coord)
        if standard_node:
    ...
    def get_node(self, coord, standard_node):
        ...
        if not free:
          if not standard_node:
            #print(coord)
            final_coords = []
            final_coords.extend(coord[MOVE_TUPLE])
            final_coords.extend(coord[0][len(coord[MOVE_TUPLE]):])
            final_coords = tuple(final_coords)
            #print(final_coords)
          else:
            final_coords = coord
          
          for c in final_coords:
            indices = [i for i, j in enumerate(final_coords) if j == c]
            if len(indices) > 1:
              self.collision_logger.add((tuple(c), tuple(indices)))
    ...
    def find_path(self, init_pos, time_limit=5 * 60):
        ...
        while len(open_list) > 0:
            ...
            for node in open_list:
              print(node.coord, node.h, node.cost)
            print('-'*10)'''
            ...
            self.nodes_popped += 1
            if not consistent:
                #print('Inconsistent Node: ', node.coord)
                continue
            node.open = False
            if self.solution_condition(node):
                ...
                return tuple(path)
            #print('-'*10)
            #print('Expanding node: ', node.coord)
    ...
    def expand(self, node, open_list):
        ...
        if len(col_set) > 0:
          for cs in col_set:
            self.max_collision_set_size = max(self.max_collision_set_size, len(cs))
        ...
        for i in neighbors:
            ...
            for coord in i.coord:
              indices = [agent_idx for agent_idx, j in enumerate(i.coord) if j == coord]
              if len(indices) > 1:
                self.collision_logger.add((tuple(coord), tuple(indices)))
        if self.epeastar:
            # if running epeastar
    ...
    def create_sub_search(self, new_goals, rob_id):
        ...
        of robots"""
        return Od_Mstar(self.obs_map, new_goals, recursive=self.recursive, model_config=self.model_config,
    ...
    def get_neighbors_recursive(self, node):
        ...
        #print('-' * 5)
        #print('Coupled Keys: ', coupled_keys)
        
        ##############################################
        prediction_already_done = False
        if self.predictor is None:
          ...
          #print('-' * 5, 'Predicted', '-' * 5)
          prediction_already_done = self.predictor.prediction_already_done(tuple(start_coord))
          if not prediction_already_done:
            # Use the predictor
            #print('Using the predictor')
            new_coord = self.predictor.predict(start_coord)
          else:
            #print('Prediction already done')
            #new_coord = list(self.predictor.already_predicted[tuple(start_coord)])
            #new_coord = list(start_coord)
            new_coord = [self.sub_search[self.policy_keys[i]].get_step(
              start_coord[i]) for i in range(self.num_bots)]
          #print(new_coord)  
        ##############################################
            ...
            try:
                if self.predictor is None:
                  ...
                  if not prediction_already_done:
                    # Use the predictor
                    #print('Using the predictor')
                    all_next_steps = self.predictor.predict(start_coord)
                  else:
                    #print('Prediction already done')
                    all_next_steps = list(self.predictor.already_predicted[tuple(start_coord)])
                    #all_next_steps = list(start_coord)
                    
                  new_step = []
                  new_step_start_coords = []
                  for j in col_set[i]:
                    new_step.append(all_next_steps[j])
                    new_step_start_coords.append(start_coord[j])
                  
                  new_step = tuple(new_step)
                  new_step_start_coords = tuple(new_step_start_coords)
                  
                  already_predicted_key = tuple([new_step_start_coords, new_step])
                  if already_predicted_key in self.odrmstar_already_predicted_next_step:
                    new_step = self.sub_search[coupled_keys[i]].get_step(
                      tuple([start_coord[j] for j in col_set[i]]))
                  else:
                    self.odrmstar_already_predicted_next_step.add(already_predicted_key)
    ...
    def get_astar_neighbors(self, node):
        ...
        if self.predictor is None:
          ...
          if not prediction_already_done:
            # Use the predictor
            #print('Using the predictor')
            base_coord = self.predictor.predict(start_coord)
          else:
            #print('Prediction already done')
            base_coord = list(self.predictor.already_predicted[tuple(start_coord)])
            #base_coord = list(start_coord)

        '''   
        for coord in base_coord:
         indices = [i for i, j in enumerate(base_coord) if j == coord]
         if len(indices) > 1:
           self.collision_logger.add((tuple(coord), tuple(indices)))
        '''
        #print('Base Coord: ', base_coord)
        old_coords = [base_coord]
        
        if self.predictor is not None and (self.predictor.prediction_already_done(tuple(base_coord)) or prediction_already_done):# or len(node.col_set) == 0):
          #print('Cycle found. Falling back to naive M* policy.')
          old_coords.append([self.sub_search[self.policy_keys[i]].get_step(
            start_coord[i]) for i in range(self.num_bots)])
        ...
        to_explore = node.col_set
        if self.full_space: 
            #print('Going to explore all neighbors...')
            to_explore = [range(self.num_bots)]
