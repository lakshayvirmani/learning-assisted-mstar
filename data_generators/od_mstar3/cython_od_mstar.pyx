...
cdef extern from "grid_planning.hpp" namespace "mstar":
    ...
    vector[vector[vector[double]]] find_cost_to_go(
        const vector[vector[bool]] &obstacles,
...
def find_all_costs_to_go(world, goals):
    import resource
    resource.setrlimit(resource.RLIMIT_AS, (2**35, 2**35)) # 8Gb
    ...
    for row in world:
        ...
        obs.push_back(temp)
    try:
        return find_cost_to_go(obs, goals)
    except Exception as e:
        if str(e) == "Out of Time":
            raise OutOfTimeError()
        elif str(e) == "No Solution":
